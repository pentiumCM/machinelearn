...
def vbb2voc(vbb_inputdir, vbb_outputdir, IMAGE_SIZE):
    """
    vbb标注文件 转 VOC标注 的 xml 入口函数
    :return:
    """
    parse_anno_file(vbb_inputdir, vbb_outputdir, IMAGE_SIZE)


if __name__ == "__main__":
    db_path = 'F:/experiment/data/KAIST/annotations'
    vbb_input = os.path.join(db_path, 'annotations-vbb')
    vbb_output = os.path.join(db_path, 'annotations-xml')

    image_size = (640, 512)  # KAIST Multispectral Benchmark

    vbb2voc(vbb_input, vbb_output, image_size)
